...
void MainWindow::createAction(){
    ...
    ctrlAction = new QAction(QIcon("Ctrl.png"), tr("控制"),this);
...
void MainWindow::multiFind(){
    ...
    for(int i = 1; i < nextPath_mid.size(); i++){   //把坐标存入路径中
        ...
        qDebug()<<"tmp:"<<tmpX<<" "<<tmpY<<"_\n";
        weight+=1.0/(num-2);
        weight_log.push_back(weight);//将数据保存下来，便于计时
...
//        printLog(strtmp);
    ...
    //Delay_MSec((1-weight)*delay);
//    Delay_MSec();
//    QString str_temp =startComboBox->itemText(nextPath_mid[nextPath_mid.size()-1]+1);
//    temp = QString::number(nextPath_mid[nextPath_mid.size()-1]+1);//获取重点的标号
//    strtmp = "用户已到：" + temp + "->" + str_temp;
//    printLog(strtmp);
