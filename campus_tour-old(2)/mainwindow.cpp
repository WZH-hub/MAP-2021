    ...
    , ui(new Ui::MainWindow)
{
    ...
    /*
    bool some=connect(ani,SIGNAL(finished()),this,SLOT(realPrint()));
    qDebug()<<"bool"<<some<<"\n";
    */
    QTimer t(this);

    //t.singleShot(0,this,SLOT(eventOperation()));
...
void MainWindow::paintPath(){
    ...
    QString strtmp,temp;
    //double weight=0.0;
    weight_log.clear();//清除之前的
    qDebug()<<"size:"<<nextPath.size()<<"_\n";

    qDebug()<<"start"<<"_\n";
    printLog("---------------------------------------------");
    screenLog("------------------------");
    //double delay = 20000.0;//总体的时延

//    connect(ani,SIGNAL(finished()),this,SLOT(realPrint()));

    QString str_temp =startComboBox->itemText(nextPath[0]+1);
    temp = QString::number(nextPath[0]+1);
    strtmp = "用户已到：" + temp + "->" + str_temp;
    printLog(strtmp);
    screenLog(strtmp);
    qDebug()<<"nextPath.size()  "<<nextPath.size()<<"_\n";
   // MainWindow::findShortestPath aMap;
    qDebug()<<tour->Map.arc[0][1].dis<<"\n";
    //int id = 1;
    ani->setStartValue(QRect(500, 500, 30, 40));
    ani->setEndValue(QRect(700, 700,30, 40));
    ani->start();
...
{
    //QMessageBox::information(this,"bb","hhh",QMessageBox::Ok);

        qDebug()<<"________________________________"<<"\n";
        qDebug()<<"idlocation"<<idLocation<<"\n";
        qDebug()<<"size"<<nextPath.size()<<"\n";
        if(idLocation<nextPath.size())
        {
            QString strtmp,temp;
            double unit_delay=20.0;
            double delay = tour->Map.arc[nextPath[idLocation-1]][nextPath[idLocation]].dis*unit_delay;
            ...
            //Delay_MSec(20);
            Delay_MSec(delay);
            QString str_temp =startComboBox->itemText(nextPath[idLocation]+1);
            temp = QString::number(nextPath[idLocation]+1);
            strtmp = "用户已到：" + temp + "->" + str_temp;
            printLog(strtmp);
            screenLog(strtmp);
...
void MainWindow::multiFind(){
    ...
    QMessageBox::information(this, "oo", "开始实时导航", QMessageBox::Ok);
    tour->findPath(startComboBox->currentIndex()-1);  //根据起点寻路
    nextPath_mid = tour->getPath(midComboBox->currentIndex()-1);
    ...
    QString strtmp,temp;
    double weight=0.0;
    int num = nextPath_mid.size()+nextPath.size();
    QVector<double> weight_log;

    for(int i = 1; i < nextPath_mid.size(); i++){   //把坐标存入路径中

        setNextPos(nextPath_mid[i]+1);

        ani->setKeyValueAt(weight+1.0/(num-2),QRect(tmpX-13, tmpY-50, 30, 40));
        qDebug()<<"tmp:"<<tmpX<<" "<<tmpY<<"_\n";
        weight+=1.0/(num-2);
        weight_log.push_back(1.0/(num-2));//将数据保存下来，便于计时
    }
    //有问题，权重！！！！！！！！！！！！！！！
    ...
    for(int i = 1; i < nextPath_mid.size(); i++)
    {
        ...
        QString str_temp =startComboBox->itemText(nextPath_mid[i]+1);
        temp = QString::number(nextPath_mid[i]+1);
        strtmp = "用户已到：" + temp + "->" + str_temp;
        printLog(strtmp);
        screenLog(strtmp);
    }
    ...
    {
        Delay_MSec(weight_log[i+(nextPath_mid.size()-1)-1]*delay);
