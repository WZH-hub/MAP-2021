...
//--------------------------------------------------------
        ...
        logicCombo->addItem("请选择需要的功能(沙河)");    //加入拥塞系数，
...
//--------------------------------------------------------
        ...
        trafficCombo->addItem("步行");    //默認交通方式為步行 0
...
void MainWindow::createAction(){
    ...
    leastTimeAct = new QAction(QIcon(), tr("最短时间"), this);
...
void MainWindow::aroundBuilding(int num)//根据全局变量idLocation找出周围的点，也就是建筑
{
...
void MainWindow::congestionControl(){
    /*
        logicCombo->addItem("运动");//1   操场——球场  logicCombo->addItem("吃饭");//2   两个食堂   logicCombo->addItem("购物");//3   小卖铺和超市
    */
    ...
    if(time == 0){  //选择第一（0）个选项
        if(logicCombo->currentText() == "吃饭"){
            ...
            QMessageBox::information(this, "吃饭建议", "根据当前时间，推荐地点为学生食堂", QMessageBox::Ok);
        }
        if(logicCombo->currentText() == "运动"){
            endComboBox->setCurrentIndex(5);
            QMessageBox::information(this, "运动建议", "根据当前时间，推荐地点为运动场", QMessageBox::Ok);
        }
        if(logicCombo->currentText() == "购物"){
            endComboBox->setCurrentIndex(15);
            QMessageBox::information(this, "购物建议", "根据当前时间，推荐地点为小麦铺", QMessageBox::Ok);
        }
    }
    else if(time == 1){     //选择第二个选项
        if(logicCombo->currentText() == "吃饭"){
            endComboBox->setCurrentIndex(9);
            QMessageBox::information(this, "吃饭建议", "根据当前时间，推荐地点为教工食堂", QMessageBox::Ok);
        }
        if(logicCombo->currentText() == "运动"){
            endComboBox->setCurrentIndex(16);
            QMessageBox::information(this, "运动建议", "根据当前时间，推荐地点为球场", QMessageBox::Ok);
        }
        if(logicCombo->currentText() == "购物"){
            endComboBox->setCurrentIndex(24);
            QMessageBox::information(this, "购物建议", "根据当前时间，推荐地点为商贸服务区", QMessageBox::Ok);
        }
    }
...
void MainWindow::paintLog()
{
    ...
    aroundBuilding(idLocation);
    emit aniSignal();//触发信号
}

void MainWindow::paintAni()
{
